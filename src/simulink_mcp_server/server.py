...
class MATLABEngineManager:
    ...
    def __init__(self):
        ...
        self.shared_engine_name = (
            "SimulinkMCP"  # Use the specific engine name you defined
        )
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            try:
                logger.info(
                    f"Attempting to connect to shared engine: {self.shared_engine_name}"
                )
        ...
        if self.matlab_available and self.is_connected and self.engine:
            ...
            try:
                ...
                cmd = [
                    matlab_exe,
                    "-batch",
                    f"run('{script_path.replace(os.sep, '/')}'); exit;",
                ]
    ...
    def _log_perf(self, code_preview: str, elapsed_ms: float, timed_out: bool):
        ...
        logger.debug(
            f"MATLAB perf: {elapsed_ms:.0f}ms {'TIMEOUT' if timed_out else 'OK'} — {code_preview}"
        )
    ...
    def get_perf_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "slow_details": [
                {"code": e["code"], "ms": e["elapsed_ms"]} for e in slow[-5:]
            ],
        }
...
def connect_simulink_engine() -> str:
    ...
    try:
        ...
        if success:
            logger.info(
                f"Successfully connected to shared MATLAB engine: {engine_manager.shared_engine_name}"
            )
            return f"Successfully connected to shared MATLAB engine '{engine_manager.shared_engine_name}' with Simulink support"
        else:
            logger.warning(
                "Failed to connect to MATLAB engine, using command-line fallback"
            )
...
def simulink_set_param(
    model_name: str, block_name: str, parameter: str, value: str
) -> str:
    ...
    try:
        ...
        if result["success"]:
            return (
                f"Successfully set parameter '{parameter}' to '{value}' for '{target}'"
            )
        else:
...
def simulink_create_subsystem(
    model_name: str, blocks: List[str], subsystem_name: str
) -> str:
...
def simulink_create_bus_selector(
    model_name: str,
    bus_signal_block: str,
...
def matlab_execute_code(
    code: str, capture_output: bool = True, timeout: Optional[int] = None
) -> Dict[str, Any]:
    ...
    try:
        if not engine_manager.engine or not engine_manager.is_connected:
            return {"status": "error", "error": "MATLAB engine not connected"}
        ...
        if capture_output:
            try:
                if timeout > 0:
                    future = engine_manager.engine.evalc(
                        code, nargout=1, background=True
                    )
                    try:
                        output = future.result(timeout=float(timeout))
                    except (TimeoutError, _MatlabTimeoutError):
            ...
            try:
                if timeout > 0:
                    future = engine_manager.engine.eval(
                        code, nargout=0, background=True
                    )
                    try:
    ...
    try:
        ...
        if hasattr(result, "__iter__") and not isinstance(result, str):
            ...
            try:
                ...
                return {"status": "success", "value": result_list, "type": "array"}
            except:
                return {"status": "success", "value": str(result), "type": "complex"}
        else:
            # Handle scalars and strings
            return {"status": "success", "value": result, "type": type(result).__name__}
...
def matlab_get_workspace_variable(variable_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        exists = engine_manager.engine.eval(
            f"exist('{variable_name}', 'var')", nargout=1
        )
        if exists == 0:
            return {
                "status": "error",
...
def matlab_list_workspace_variables() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "success", "variables": var_names, "details": whos_output}
...
def matlab_clear_workspace(variables: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if variables:
            ...
            return {"status": "success", "message": f"Cleared variables: {var_list}"}
        else:
            # Clear all variables
            engine_manager.engine.eval("clear", nargout=0)
            return {"status": "success", "message": "Cleared all workspace variables"}
...
def matlab_run_script(script_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "success", "output": str(output), "script": script_path}
